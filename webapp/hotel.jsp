...
<%
	...
	if(btnval.equalsIgnoreCase("save"))
	{
		...
		try
		{
		    ...
		    ResultSet rs=smt.executeQuery("select emp_id from emp_info"); // rs is the buffer
		    //Employee ID as foreign key
		    int empidfk=0;
		    while(rs.next())
			{
		    ...
		    if(empidfk==0)
				...
				{
					PreparedStatement psmt=conn.prepareStatement("insert into hotel values(?,?,?,?,?,?)");
					psmt.setString(1,t1);
					psmt.setString(2,t2);
					psmt.setString(3,t3);
					psmt.setString(4,t4);
	...
	if(btnval.equalsIgnoreCase("delete"))
	{
		...
		try
		{
			Class.forName("oracle.jdbc.driver.OracleDriver");
			Connection conn=DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe", "hotel", "hotel");
			PreparedStatement psmt=conn.prepareStatement("delete from hotel where hotel_code=?");
			psmt.setString(1,t1);
			psmt.executeQuery();
	...
	if(btnval.equalsIgnoreCase("update"))
        ...
        String t6 = request.getParameter("t6");
			...
			try
			{
				Class.forName("oracle.jdbc.driver.OracleDriver");
				Connection conn=DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe", "hotel", "hotel");
				PreparedStatement psmt=conn.prepareStatement("update hotel set emp_id=?,location=?,hotel_name=?,created_date=?,created_by=? where hotel_code=?");
				psmt.setString(1,t2);
				psmt.setString(2,t3);
				psmt.setString(3,t4);
				psmt.setString(4,t5);
			 ...
			 try
			  {
				  ...
				  Statement smt=conn.createStatement();
				  ResultSet rs=smt.executeQuery("select * from hotel"); // rs is the buffer
				  %>
				  <center>
				  <table border=2>
				  <tr>
			 ...
			 try
			  {
				  ...
				  Statement smt=conn.createStatement();
				  ResultSet rs=smt.executeQuery("select * from hotel where hotel_code='"+t1+"'"); // rs is the buffer
				  %>
				  <center>
				  <table border=2>
				  <tr>
			...
			try
			  {
				  ...
				  if(col.equalsIgnoreCase("all"))
				  	  rs=smt.executeQuery("select * from hotel");
				  else
					  rs=smt.executeQuery("select * from hotel where "+col+ "=" +"'"+t1+"'");
				%>
	              <center>
				  <table border=2>
				  <tr>
