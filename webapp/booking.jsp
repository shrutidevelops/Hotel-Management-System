		...
		if(btnval.equalsIgnoreCase("update"))
	        ...
	        String t9 = request.getParameter("t9");
				try
				{
					Class.forName("oracle.jdbc.driver.OracleDriver");
					Connection conn=DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe", "hotel", "hotel");
					PreparedStatement psmt=conn.prepareStatement("update booking set room_id=?,customer_id=?,check_in=?,check_out=?,phone_no=?,reference=?,created_date=?,created_by=? where booking_id=?");
					psmt.setString(1,t2);
					psmt.setString(2,t3);
					psmt.setString(3,t4);
					psmt.setString(4,t5);
				 ...
				 try
				  {
					  ...
					  ResultSet rs=smt.executeQuery("select * from booking"); // rs is the buffer
					  %>
					  <center>
					  <table border=2>
					  <tr>
				 ...
				 try
				  {
					  ...
					  Statement smt=conn.createStatement();
					  ResultSet rs=smt.executeQuery("select * from booking where booking_id='"+t1+"'"); // rs is the buffer
					  %>
					  <center>
					  <table border=2>
					  <tr>
				...
				try
				  {
					  ...
					  if(col.equalsIgnoreCase("all"))
						  ...
						  rs=smt.executeQuery("select * from booking where "+col+ "=" +"'"+t1+"'");
					%>
					<center>
					  <table border=2>
					  <tr>
