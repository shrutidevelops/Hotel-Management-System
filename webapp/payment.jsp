...
<%
	...
	if(btnval.equalsIgnoreCase("save"))
	{
		...
		try
		{
			...
			PreparedStatement psmt=conn.prepareStatement("insert into payment values(?,?,?,?,?,?,?)");
			psmt.setString(1,t1);
			psmt.setString(2,t2);
			psmt.setString(3,t3);
			psmt.setString(4,t4);
	...
	if(btnval.equalsIgnoreCase("delete"))
	{
		...
		try
		{
			Class.forName("oracle.jdbc.driver.OracleDriver");
			Connection conn=DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe", "hotel", "hotel");
			PreparedStatement psmt=conn.prepareStatement("delete from payment where payment_id=?");
			psmt.setString(1,t1);
			psmt.executeQuery();
			...
			if(btnval.equalsIgnoreCase("update"))
		        ...
		        String t7 = request.getParameter("t7");
					try
					{
						Class.forName("oracle.jdbc.driver.OracleDriver");
						Connection conn=DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe", "hotel", "hotel");
						PreparedStatement psmt=conn.prepareStatement("update payment set invoice_id=?,payment_date=?,total_amt=?,paid_date=?,created_date=?,created_by=? where payment_id=?");
						psmt.setString(1,t2);
						psmt.setString(2,t3);
						psmt.setString(3,t4);
						psmt.setString(4,t5);
					 ...
					 try
					  {
						  ...
						  ResultSet rs=smt.executeQuery("select * from payment"); // rs is the buffer
						  %>
						  <center>
						  <table border=2>
						  <tr>
					 ...
					 try
					  {
						  ...
						  Statement smt=conn.createStatement();
						  ResultSet rs=smt.executeQuery("select * from payment where payment_id='"+t1+"'"); // rs is the buffer
						  %>
						  <center>
						  <table border=2>
						  <tr>
					...
					try
					  {
						  ...
						  if(col.equalsIgnoreCase("all"))
						  	  rs=smt.executeQuery("select * from payment");
						  else
							  rs=smt.executeQuery("select * from payment where "+col+ "=" +"'"+t1+"'");
						%>
						<center>
						  <table border=2>
						  <tr>
